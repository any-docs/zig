...
::: code-group
```zig [test_single_item_pointer.zig]
...
::: code-group
```zig [test_pointer_arithmetic.zig]
const expect = @import("std").testing.expect;
...
In Zig, we generally prefer [Slices](/slices) rather than [Sentinel-Terminated Pointers](/slices#sentinel-terminated-pointers). You can turn an array or pointer into a slice using slice syntax.
...
::: code-group
```zig [test_comptime_pointers.zig]
const expect = @import("std").testing.expect;
...
::: code-group
```zig [test_integer_pointer_conversion.zig]
const expect = @import("std").testing.expect;
...
::: code-group
```zig [test_comptime_pointer_conversion.zig]
const expect = @import("std").testing.expect;
...
[@ptrCast](/builtin-functions#@ptrCast) converts a pointer's element type to another. This creates a new pointer that can cause undetectable Illegal Behavior depending on the loads and stores that pass through it. Generally, other kinds of type conversions are preferable to @ptrCast if possible.

::: code-group
```zig [test_pointer_casting.zig]
...
See also:
- [Optional Pointers](/optionals#pointers)
- [@ptrFromInt](/builtin-functions#@ptrFromInt)
- [@intFromPtr](/builtin-functions#@intFromPtr)
...
## Alignment
Each type has an `alignment` - a number of bytes such that, when a value of the type is loaded from or stored to memory, the memory address must be evenly divisible by this number. You can use [@alignOf](/builtin-functions#@alignOf) to find out this value for any type.
...
::: code-group
```zig [test_variable_alignment.zig]
const std = @import("std");
...
In the same way that a `*i32` can be [coerced](/casting#type-coercion) to a *const i32, a pointer with a larger alignment can be implicitly cast to a pointer with a smaller alignment, but not vice versa.

You can specify alignment on variables and functions. If you do this, then pointers to them get the specified alignment:

::: code-group
```zig [test_variable_func_alignment.zig]
const expect = @import("std").testing.expect;
...
If you have a pointer or a slice that has a small alignment, but you know that it actually has a bigger alignment, use [@alignCast](/builtin-functions#@alignCast) to change the pointer into a more aligned pointer. This is a no-op at runtime, but inserts a [safety check](/illegal-behavior#incorrect-pointer-alignment):

::: code-group
```zig [test_incorrect_pointer_alignment.zig]
const std = @import("std");
...
## allowzero
This pointer attribute allows a pointer to have address zero. This is only ever needed on the freestanding OS target, where the address zero is mappable. If you want to represent null pointers, use Optional Pointers instead. [Optional Pointers](/optionals#pointers) with `allowzero` are not the same size as pointers. In this code example, if the pointer did not have the `allowzero` attribute, this would be a [Pointer Cast Invalid Null](/illegal-behavior#pointe-cast-invalid-null) panic:

::: code-group
```zig [test_allowzero.zig]
const std = @import("std");
const expect = std.testing.expect;
...
::: code-group
```zig [sentinel-terminated_pointer.zig]
const std = @import("std");
...
See also:
- [Sentinel-Terminated Slices](/slices#sentinel-terminated-slices)
