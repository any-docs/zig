...
::: code-group
```zig [struct_default_field_values.zig]
const Foo = struct {
a: i32 = 1234,
b: i32,
};
...
See [packed struct](/struct#packed-struct) for a struct that has the ABI of its backing integer, which can be useful for modeling flags.

See also:
- [extern union](/union#extern-union)
- [extern enum](/enum#extern-enum)
...
- A [bool](/values#primitive-types) field uses exactly 1 bit.
- An [enum](/enum) field uses exactly the bit width of its integer tag type.
- A [packed union](/union#packed-union) field uses exactly the bit width of the union field with the largest bit width.
- A `packed struct` field uses the bits of its backing integer.

This means that a packed struct can participate in a [@bitCast](/builtin-functions#@bitCast) or a [@ptrCast](/builtin-functions#@ptrCast) to reinterpret memory. This even works at [comptime](/comptime):

::: code-group
```zig [test_packed_structs.zig]
const std = @import("std");
...
::: code-group
```zig [test_missized_packed_struct.zig]
...
::: code-group
```zig [test_pointer_to_non-byte_aligned_field.zig]
const std = @import("std");
const expect = std.testing.expect;
...
::: code-group
```zig [test_misaligned_pointer.zig]
const std = @import("std");
const expect = std.testing.expect;
...
::: code-group
```zig [test_packed_struct_field_address.zig]
const std = @import("std");
const expect = std.testing.expect;
...
This can be observed with [@bitOffsetOf](/builtin-functions#@bitOffsetOf) and [@offsetOf](/builtin-functions#@offsetOf):

::: code-group
```zig [test_bitOffsetOf_offsetOf.zig]
const std = @import("std");
const expect = std.testing.expect;
...
::: code-group
```zig [test_overaligned_packed_struct.zig]
const std = @import("std");
const expect = std.testing.expect;
...
::: code-group
```zig [test_aligned_struct_fields.zig]
const std = @import("std");
...
::: code-group
```zig [test_packed_struct_equality.zig]
const std = @import("std");
const expect = std.testing.expect;
...
Field access and assignment can be understood as shorthand for bitshifts on the backing integer. These operations are not [atomic](/atomic), so beware using field access syntax when combined with memory-mapped input-output (MMIO). Instead of field access on [volatile Pointers](/pointers#volatile), construct a fully-formed new value first, then write that value to the volatile pointer.

::: code-group
```zig [packed_struct_mmio.zig]
...
The fields are implicitly named using numbers starting from 0. Because their names are integers, they cannot be accessed with . syntax without also wrapping them in `@""`. Names inside `@""` are always recognised as [identifiers](/variables#identifiers).

Like arrays, tuples have a .len field, can be indexed (provided the index is comptime-known) and work with the ++ and ** operators. They can also be iterated over with [inline for](/for#inline).
...
Tuples can be [destructured](/values#destructuring).

Tuple destructuring is helpful for returning multiple values from a block:

::: code-group
```zig [destructuring_block.zig]
...
::: code-group
```zig [destructuring_return_value.zig]
const print = @import("std").debug.print;
...
- [Destructuring Arrays](/arrays#destructuring)
- [Destructuring Vectors](/vectors#destructuring)

See also:
- [comptime](/comptime)
- [@fieldParentPtr](/builtin-functions#fieldParentPtr)